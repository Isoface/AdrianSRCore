...
public class CustomBossBar implements BossBar {
	...
	public void setProgress(double progress) {
		// check progress
		Preconditions.checkArgument((progress >= 0.0D && progress <= 1.0D), 
		...
		// check progres. (Max and Min).
		progress = Math.min(Math.max((float) progress, 
				MIN_BOSS_BAR_PROGRESS), MAX_BOSS_BAR_PROGRESS);
		
		// get and set progress.
		final float parcent = (float) ((progress * 100.0F) / 1.0F);
		this.progress = (parcent * wither.getMaxHealth()) / 100;
