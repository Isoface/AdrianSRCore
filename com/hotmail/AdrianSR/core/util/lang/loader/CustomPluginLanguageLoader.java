 ...
 * TODO: Description
 * <p>
 * @author AdrianSR / Date: 19 oct. 2019 / Time: 4:53:24 p. m.
 */
public class CustomPluginLanguageLoader {
	...
	public CustomPluginLanguageLoader(CustomPlugin plugin, JarFile jar) {
		Validate.notNull(plugin, "The plugin cannot be null!");
		Validate.notNull(jar, "The jar cannot be null!");
		Validate.notNull(plugin.getLanguageContainer(), "The language enum container of the plugin cannot be null!");
		Validate.isTrue(!StringUtils.isBlank(plugin.getLanguageResourcesPackage()), "The language resources package of the plugin cannot be blank!");
		
		this.plugin            = plugin;
		this.jar               = jar;
		this.container         = plugin.getLanguageContainer();
		this.resources_package = plugin.getLanguageResourcesPackage();
	}
		...
		for (JarEntry entry : jar.stream().filter(other -> other.getName().startsWith(package_path)).collect(Collectors.toList())) {
			...
			try {
				...
				while( ( line = reader.readLine() ) != null ) {
					builder.append(line);
					builder.append(TextUtils.LINE_SEPARATOR_CHAR);
				}
				return loadFromString(entry.getName(), builder.toString());
			} catch (IOException | InvalidPluginLangConfigurationException e) {
				PrintUtils.print(ChatColor.RED, String.format(
	...
	public boolean loadFromString(String file_name, String contents) throws InvalidPluginLangConfigurationException {
		String[]       lines = contents.split(String.valueOf(TextUtils.LINE_SEPARATOR_CHAR));
		...
		for (String line : lines) {
			...
			if (val_indi_index == -1) {
				throw new InvalidPluginLangConfigurationException(
			...
			if (locale == null) { // load locale
				...
				Enum<? extends CustomPluginLanguageEnumContainer> item_constant = Arrays.stream(container.getEnumConstants()).filter(
						constant -> key.equalsIgnoreCase(((CustomPluginLanguageEnumContainer) constant).getKey()))
						.findAny().orElse(null);
				if (item_constant != null) {
					((CustomPluginLanguageEnumContainer) item_constant).setValue(value, locale);
				}
			}
		}
		
		if (locale == null) {
			throw new InvalidPluginLangConfigurationException(String.format(
