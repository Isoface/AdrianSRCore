...
public class YamlConfigurationComments extends YamlConfiguration {
    ...
    protected String insertComments(String dump) {
    	...
    	for (int i = 0; i < lines.length; i++) {
			...
			if (i > 0 && indent >= options().indent() && line.indexOf(VALUE_INDICATOR) != -1) {
				for (int j = i; j >= 0; j--) {
					...
					if (line_back.trim().startsWith(COMMENT_PREFIX)
							|| line_back.trim().startsWith(YamlConfigurationCommentsOptions.PATH_COMMENT_PREFIX)) {
						continue;
					}
			...
			ConfigurationSection section = node.isEmpty() ? this 
					: getConfigurationSection(node.substring(0, node.lastIndexOf(options().pathSeparator())));
			if (section != null) {
				...
				if (options().isCommented(section, line_path)) {
					builder.append(getIndent(indent) + YamlConfigurationCommentsOptions.PATH_COMMENT_PREFIX + options().getComment(section, line_path));
    ...
    protected void loadComments(String input) {
    	...
    	for (int i = 0; i < lines.length; i++) {
			...
			if (!comment_line.trim().startsWith(YamlConfigurationCommentsOptions.PATH_COMMENT_PREFIX)
					|| comment_line.trim().length() == YamlConfigurationCommentsOptions.PATH_COMMENT_PREFIX.length()) {
				continue;
			}
			
			String comment = comment_line.trim().substring(YamlConfigurationCommentsOptions.PATH_COMMENT_PREFIX.length()).trim();
	...
	public YamlConfigurationCommentsOptions options() {
		return (YamlConfigurationCommentsOptions) (options == null ? (options = new YamlConfigurationCommentsOptions(this))
				: options);
	}
}