 ...
 * and allows to comments the keys by using the method {@link #options()}.comment().
 * <p>
 * Note that this implementation is not synchronized.
 * <p>
 * @author AdrianSR / Date: 10 oct. 2019 / Time: 5:31:28 p. m.
 */
public class CustomYamlConfiguration extends YamlConfiguration {
    ...
    /**
     * Creates a new {@link CustomYamlConfiguration}, loading from the given file.
     * <p>
    ...
    /**
     * Creates a new {@link CustomYamlConfiguration}, loading from the given reader.
     * <p>
     * Any errors loading the Configuration will be logged and then ignored.
     * If the specified input is not a valid config, a blank config will be
     * returned.
     ...
     */
    public static CustomYamlConfiguration loadConfiguration(Reader reader) {
    ...
    public String saveToString() {
        ...
        dump = dump.substring(0, dump.lastIndexOf(LINE_SEPARATOR)); // clear last line
        return header + dump;
    }
    ...
    protected String insertComments(String dump) {
    	...
    	for (int i = 0; i < lines.length; i++) {
			...
			if (i > 0 && indent >= options().indent() && line.indexOf(VALUE_INDICATOR) != -1) {
				for (int j = i; j >= 0; j--) {
					...
					if (line_back.trim().startsWith(COMMENT_PREFIX)
							|| line_back.trim().startsWith(CustomYamlConfigurationOptions.PATH_COMMENT_PREFIX)) {
						continue;
					}
					...
					if ( line_back_indent == ( indent - options().indent() ) ) {
						...
						line_back_path = line_back.trim().substring(0, line_back.trim().indexOf(VALUE_INDICATOR));
			...
			ConfigurationSection section = node.isEmpty() ? this : getConfigurationSection(node.substring(0, node.lastIndexOf(options().pathSeparator())));
			if (section != null) {
				...
				if (options().isCommented(section, line_path)) {
					builder.append(getIndent(indent) + CustomYamlConfigurationOptions.PATH_COMMENT_PREFIX + options().getComment(section, line_path));
    ...
    /**
     * Gets the equivalent of the given indent
     ...
     *  - getIndent(6) = "      "
     * <p>
     * @param indent_count the indent count.
    ...
    protected void loadComments(String input) {
    	...
    	for (int i = 0; i < lines.length; i++) {
						...
						if ( line_back_indent == ( indent - options().indent() ) ) {
							...
							if (last_node.contains(String.valueOf(options().pathSeparator()))) {
								...
								if (last_node.substring(0, last_node.lastIndexOf(options().pathSeparator()))
									...
									node = last_node.substring(0, last_node.lastIndexOf(options().pathSeparator()))
			...
			if (!comment_line.trim().startsWith(CustomYamlConfigurationOptions.PATH_COMMENT_PREFIX)
					|| comment_line.trim().length() == CustomYamlConfigurationOptions.PATH_COMMENT_PREFIX.length()) {
				continue;
			}
			
			String comment = comment_line.trim().substring(CustomYamlConfigurationOptions.PATH_COMMENT_PREFIX.length()).trim();
			if (node.lastIndexOf(options().pathSeparator()) == -1) { // root section
				options().comment(this, line_path, comment);
				continue;
			}
			
			String parent_node = node.substring(0, node.lastIndexOf(options().pathSeparator()));
			if (isConfigurationSection(parent_node) && !isConfigurationSection(node)) { // sub section
    ...
    /**
     * Counts the amount of indent at the
     * start of a line.
     * <p>
	...
	@Override
	public CustomYamlConfigurationOptions options() {
		return (CustomYamlConfigurationOptions) (options == null ? (options = new CustomYamlConfigurationOptions(this))
				: options);
	}
}