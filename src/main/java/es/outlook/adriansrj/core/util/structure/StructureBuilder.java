 ...
 * TODO: Description
 * <br>
 * @author AdrianSR / Saturday 13 February, 2021 / 09:01 AM
 */
public class StructureBuilder {
	...
	public StructureBuildTask build ( World world ) {
		...
		Bukkit.getScheduler ( ).runTask ( AdrianSRCore.getInstance ( ) , build_task );
		return build_task;
	}
	...
	public StructureBuildTask buildAsynchronously ( World world ) {
		...
		Bukkit.getScheduler ( ).runTaskAsynchronously ( AdrianSRCore.getInstance ( ) , build_task );
		return build_task;
	}
	 ...
	 * TODO: Description
	 * <br>
	 * @author AdrianSR / Saturday 13 February, 2021 / 09:28 AM
	 */
	protected static class StructureBuildTask implements Runnable {
		
		// TODO: implement a feature that allows developers to specify a priority by type, so some blocks will be
		//  placed before others to avoid bugs
		...
		protected void set ( BlockVector position , Material type ) {
			...
			Bukkit.getScheduler ( ).scheduleSyncDelayedTask ( AdrianSRCore.getInstance ( ) , new Runnable ( ) {
				@Override
				public void run ( ) {
					block.setType ( type );
