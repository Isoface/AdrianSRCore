...
public class ColorUtil {
	...
	private static final Pattern cssAbsolutePattern = Pattern
			.compile( COLOR_CSS_PATTERN );

	/**
	 * Compiled pattern for CSS relative pattern: "RGB( 255%, 0%, 0% )"
	 */
	private static final Pattern cssRelativePattern = Pattern
	...
	private static String hexToRGB( String hexColor, boolean isAbsolute )
	{
		...
		else
		{
			...
			sb
					.append( "RGB(" ).append( r_fValue ).append( "%," ); //$NON-NLS-1$ //$NON-NLS-2$
			sb.append( g_fValue ).append( "%," ); //$NON-NLS-1$
			sb.append( b_fValue ).append( "%)" ); //$NON-NLS-1$
		}
