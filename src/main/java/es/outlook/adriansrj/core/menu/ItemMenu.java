...
import es.outlook.adriansrj.core.menu.holder.ItemMenuHolder;
...
public class ItemMenu {
	...
	public ItemMenu ( String title , ItemMenuSize size , Item... contents ) {
		this ( title , size , null , contents );
	}
	...
	public boolean isMenuOpen ( Player player ) {
		...
		return this.equals (
				( ( ItemMenuHolder ) player.getOpenInventory ( ).getTopInventory ( ).getHolder ( ) ).getItemMenu ( ) );
	}
	...
	public ItemMenu setSize ( ItemMenuSize size ) {
		...
		Item[] contents_copy = new Item[ this.contents.length ];
		System.arraycopy ( contents , 0 , contents_copy , 0 , contents.length );
		
		this.contents = new Item[ size.getSize ( ) ];
		this.size     = size;
		
		// migrating
		System.arraycopy ( contents_copy , 0 , contents , 0 , contents.length );
		return this;
	}
	...
	/**
	 * Gets whether listeners are registered for this menu.
	 *
	 * @return whether listeners are registered for this menu.
	 */
	...
	public boolean registerListener ( Plugin plugin ) {
		if ( this.handler == null ) {
			this.handler = new ItemMenuHandler ( this , plugin );
	...
	public Inventory open ( Player player ) {
		Inventory inventory = apply (
				Bukkit.createInventory (
						new ItemMenuHolder ( this , Bukkit.createInventory ( player , size.getSize ( ) ) ) ,
						size.getSize ( ) ,
						StringUtil.limit ( StringUtil.translateAlternateColorCodes ( getTitle ( ) ) , 32 ) ) );
	...
	public ItemMenu onClick ( final ItemMenuClickAction action ) {
		...
		ItemClickAction sub_action = new ItemClickAction (
				this , action.getInventoryView ( ) ,
				action.getClickType ( ) , action.getInventoryAction ( ) ,
				action.getSlotType ( ) , action.getSlot ( ) ,
				action.getRawSlot ( ) , action.getCurrentItem ( ) ,
				action.getHotbarKey ( ) , false , false , false );
