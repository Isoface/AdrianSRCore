...
public class ImageMessage implements Cloneable {
	...
	private       int           height;
	private final ImageChar image_char;

	public ImageMessage(BufferedImage image, int height, ImageChar image_char) {
		this.lines      = toImgMessage(toChatColorArray(image, height), image_char);
		this.image      = image;
		this.height     = height;
		this.image_char = image_char;
	}

	public ImageMessage(ChatColor[][] chatColors, ImageChar image_char) {
		this.lines      = toImgMessage(chatColors, image_char);
		this.image_char = image_char;
	}

	public ImageMessage(String... imgLines) {
		this.lines      = imgLines;
		this.image_char = ImageChar.TRANSPARENT_CHAR;
	}
	...
	private String[] toImgMessage(ChatColor[][] colors, ImageChar image_char) {
		String[] lines = new String[colors[0].length];
		for (int y = 0; y < colors[0].length; y++) {
			String line = "";
			for (int x = 0; x < colors.length; x++) {
				ChatColor color = colors[x][y];
				line += (color != null) ? colors[x][y].toString() + image_char.getChar() : ImageChar.TRANSPARENT_CHAR.getChar();
			}
