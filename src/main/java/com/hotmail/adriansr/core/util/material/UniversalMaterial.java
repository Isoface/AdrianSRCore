...
*/
@SuppressWarnings("deprecation") public enum UniversalMaterial {
    ...
    */
    ...
    private static final HashMap<String, UniversalMaterial> inUMemory = new HashMap<>();
	...
	UniversalMaterial(String... names) {
		this(0, names);
	}

	UniversalMaterial(String name, int data) {
		names[0] = name;
		versionName = name;
		this.data = (byte) data;
	}

	UniversalMaterial(String name, int spoofedData, Color color) {
		names[0] = name;
		versionName = name;
	...
	UniversalMaterial(PotionBase base, String type, boolean extended, boolean upgraded, String... names) {
		this.names = names;
		attributes = "upotion=" + base.name() + ":" + type + ":" + extended + ":" + upgraded;
	}

	UniversalMaterial(String name, Enchantment enchant, int level) {
		names[0] = name;
		versionName = name;
		data = 0;
	...
	UniversalMaterial(int data, String... names) {
		this.names = names;
		this.data = (byte) data;
	}
    ...
    @Deprecated
    public static UniversalMaterial match(String name, byte data) {
        name = name.toUpperCase();
        if(inUMemory.containsKey(name + data)) return inUMemory.get(name + data);
        for(UniversalMaterial u : UniversalMaterial.values()) {
            if(u.getData() == data) {
	...
	public static UniversalMaterial matchSpawnEgg(ItemStack egg) {
		if (egg != null) {
    ...
    public static UniversalMaterial matchEnchantedBook(ItemStack book) {
    ...
    public static UniversalMaterial matchPotion(ItemStack potion) {
	...
	public static UniversalMaterial match(ItemStack item) {
		if (item != null && !item.getType().equals(Material.AIR)) {
			...
			if (version.contains("1.8") || version.contains("1.9") || version.contains("1.10")
				...
				final UniversalMaterial u = match(un, d), potion = u == null ? matchPotion(item) : null;
	...
	public static UniversalMaterial getItem(Block block) {
		...
		if (!v.contains("1.8") && !v.contains("1.9") && !v.contains("1.10") && !v.contains("1.11")
			...
			if (d == 8) {
				t = UniversalMaterial.ACACIA_WOOD;
			} else if (d == 12) {
				t = log2 ? UniversalMaterial.ACACIA_WOOD : UniversalMaterial.OAK_WOOD;
			} else if (d == 13) {
				t = log2 ? UniversalMaterial.DARK_OAK_WOOD : UniversalMaterial.SPRUCE_WOOD;
			} else if (d == 14) {
				t = UniversalMaterial.BIRCH_WOOD;
			} else if (d == 15) {
				t = UniversalMaterial.JUNGLE_WOOD;
			}
			...
			if (v.contains("1.12")) {
				...
				return UniversalMaterial.WHITE_BED;
		} else if (m.contains("CROP")) {
			return UniversalMaterial.AIR;
		} else if (m.contains("PISTON_")) {
			return m.contains("_STICKY_") ? UniversalMaterial.STICKY_PISTON : UniversalMaterial.PISTON;
...
@SuppressWarnings("deprecation") class UPotion {
    ...
    private final UniversalMaterial.PotionBase base;
    private final ItemStack potion;
    private final PotionType type;
    private final Object potiondata;
    
    public UPotion(UniversalMaterial.PotionBase base, String type, boolean extended, boolean upgraded) {
        this.base = base;
	...
	public UniversalMaterial.PotionBase getBase() {
		return base;
	}
